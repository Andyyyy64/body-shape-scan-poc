...
3. 撮影時間を設定し、「5秒後に撮影開始」で一周する。回り方・速さは自由だが、お腹は脱力して普通に呼吸し、腕は体から少し離して下げ、カメラから約2mで頭から腰までを常に画面に入れる（撮影条件`relaxed_arms_down_full_torso.v1`）。服装は毎回同じにする。
4. 保存した撮影の「3Dに復元」を押す。処理は1件ずつ実行する。
5. 完了した撮影をクリックすると、原動画と1回分の3Dを確認できる。
6. 2回目も別に撮影・復元し、基準と比較対象を選んで「差分を計算」を押す。

同じ服装・距離で撮り直し、まず1〜4時間内のばらつきを調べる。日付によるゼロ補正や、前回の体型をそのまま返す処理はない。許容幅は任意の手動設定で、検証済みの生理的変化の閾値ではない。
...
動画を実際にデコードしてフレーム数を数え、既定では16枚を選ぶ。ブラウザ録画のシーク情報やFPSだけに依存しない。各フレームでYOLOの人物マスクを取り、複数の検出がある場合は最大面積の人物を採用する。ただし2番目以降の検出が最大面積の25%を超える（別の人物と区別できない）フレームは`multiple_people`として捨てる。バッグなど静止物の低信頼検出はこれで無視される。

採用した人物にSAM 3D Bodyを適用し、各フレームの姿勢・形状・カメラ並進・画像上の関節を得る。関節から撮影条件を検査し、頭・両肩・両腰のいずれかが画面外（`core_out_of_frame`）、または手首が肘より上（腕上げ・腕組み・力こぶ、`arm_not_lowered`）のフレームは復元から除外する。除外後に残るフレームがサンプル数の75%（16枚なら12枚）未満なら、meshを作らずに理由を示して撮り直しを求める。除外数と理由は`reconstruction.json`と画面の一覧に出す。

残ったフレームのshape係数の中央値を初期値とし、各フレームの姿勢・骨格尺度・焦点距離を固定したまま、1つの共有shapeと各フレームのカメラ並進を人物輪郭へ最適化する（`fixed_pose_silhouette_fit.v1`）。1回目に骨格スケール、正準姿勢の断面位置、胴体の対象頂点を`profile.json`へ記録し、その後の撮影で共有する。MHRを同じ姿勢で再生成し、同じ頂点同士を比較する。任意の拡大縮小や非剛体位置合わせで差を消さない。他の撮影の形状、日時、目標周囲長は入力しない。

これは固定した初期姿勢と輪郭頂点集合を使う局所的な形状最適化で、姿勢と形状の完全な共同推定ではない。合成データでは幾何が同一でも一定のオフセットが残るため、この方式は差分（撮影間の比較）にのみ使い、絶対寸法には使わない。向きが不足した場合は警告し、同じ原動画からの再処理も区別する。比較値を脂肪・筋肉の変化とは断定しない。

実モデルについて確認したのは、公開サンプルでの推論、SAMの完全なMHRパラメータからの頂点再生成、公開画像由来の同じ動画を2回処理する接続試験、および本人の短時間内の複数撮影による非公開の再現性評価。実寸精度と、1cmの実変化に対する感度は未検証。
...
輪郭フィットが既定になる前に`sam_parameter_ensemble`で復元した撮影は、比較方式が異なるため新しい撮影と直接比較できない。元の動画・meshを変更せず、同じ保存先に「輪郭フィット（再処理）」という新しい撮影レコードを作るには次を明示的に実行する。人物選択と撮影条件の除外は通常の復元と同じ規則を適用する。
...
初期・最終の形状パラメータ、画像上の残差を非公開で保存する。

合成データで形状変化への感度・撮影順序への不変性・人物選択と撮影条件の規則を確認するテストはモデル環境で実行する。

```sh
"$MODEL_PYTHON" -m unittest tests.test_shared_shape tests.test_contours tests.test_quality -v
```
